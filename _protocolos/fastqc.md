...
**10**.	O gráfico de **distribuição do tamanho das sequências** irá traduzir o tamanho dos fragmentos da biblioteca e/ou o tamanho máximo dos reads gerados na plataforma. Para a plataforma Illumina, é normal o gráfico com um pico nos tamanhos de 101, 151, 251 ou 301 bases (a depender da plataforma).

**11**.	O módulo de **análise de sequências duplicadas** indica a quantidade de reads idênticos gerados na biblioteca. O resultado pode indicar um alto nível de cobertura de um determinado genoma (regiões sequenciadas com redundância) ou algum viés na construção ou enriquecimento da biblioteca. É esperado que em bibliotecas suficientemente diversas, a maior parte das sequências esteja à esquerda do gráfico. Em caso de enriquecimento de bibliotecas, algumas regiões podem estar super-representadas e por isso picos ao longo da curva podem ocorrer. O módulo também calcula a perda de reads em caso de deduplicação (remoção dos reads duplicados). 
...
**13**.	A análise do **conteúdo de adaptadores** indica a presença (ou não) dos mesmos nas sequências. Várias plataformas utilizam a ligação de adaptadores durante a construção das bibliotecas e eles são sequenciados juntamente com a sequência de interesse. É comum realizar um pós-processamento das sequências com programas para remover os adaptadores, bases e reads com baixa qualidade (Trimmomatic, Cutadapt, fastp, BBDuk são muito utilizados). Quando há detecção de adaptadores nos reads, o gráfico sempre os colocará no final e a porcentagem sempre aumentará no fim da curva, quando presentes.
...
<div align="center">
<img src="/metodos/images/fastq.png" alt="fastq" width="100%;">
</div>
